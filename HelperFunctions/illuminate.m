...
%        [w screenRect]=Screen('OpenWindow',screenNumber, gray, [0 0 400 400]);
    ...
    Help{7} = 'z: decrease luminance';
    Help{8} = 'x: increase luminance';
...
%    DrawMultiLineComment(screen, Help, 'x0', 300)
    ...
    while 1
        ...
        increase_luminance = KbName('x');
        decrease_luminance = KbName('z');
        ...
        elseif keyCode(increase_luminance)
            if color(1)<255
                color = color + [1 1 1];
            end
        elseif keyCode(decrease_luminance)
            if color(1)>0
                color = color - [1 1 1];
            end
