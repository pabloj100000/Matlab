function q = HeightDiscriminationTask(varargin)
...
try
    ...
    p.addParamValue('objHeight', 10, @(x) x>0);
    p.addParamValue('color', 100, @(x) x>=0 && x<=255);
    p.addParamValue('pedestal', 0, @(x) x>=0);
    
    % other parameters
    p.addParamValue('stimSize', [], @(x) x>0);      % area occupied by background checkers
    p.addParamValue('backMaskSize', 150, @(x) isnumeric(x)); % dimension of the gray screen covering part of the checkers
    ...
    % gabor parameters
    objSize = [p.Results.objSize p.Results.objHeight];
    ...
    % Get object texture
    [gaussTex] = GetGaussianDisk(objSize, 127, 'decay', 5);

    % Make all necessary rectangles
    objDest = CenterRectOnPoint(SetRect(0, 0, objSize(1), objSize(2)), screen.center(1)-deltaX, screen.center(2))';
    objDest(:,2) = CenterRectOnPoint(SetRect(0, 0, objSize(1), objSize(2)), screen.center(1)+deltaX, screen.center(2))';
    ...
    % Get 1 rectangles for the fixation point
    ...
    Screen('DrawText', screen.w, 'Hieght Task', 0, 0, 0, 127);
    Screen('DrawText', screen.w, 'Your job is to select the patch with the highest bar', 0, 20, 0, 127);
    ...
    for i=1:trialsPerDelay
        ...
        for j=1:objDelayN
            ...
            testedShift = round(exp(tTest)*DiffSign+pedestal);
            
            if pedestalPatch==1
                testRects = objDest+testedShift*[0 0 0 0;0 1 0 1]';
            else
                testRects = objDest+testedShift*[0 1 0 1;0 0 0 0]';
            end
            ...
            for frame = 0:framesN
                ...
                if (objDelayFrame + saccadeFrame < frame && frame < objDelayFrame+flashObjFrames + saccadeFrame)
                    Screen('FillRect', screen.w, color, testRects);
                    Screen('BlendFunction',screen.w, GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
                    Screen('DrawTextures', screen.w, gaussTex, [], testRects);
                end
            ...
            fprintf('\t%s, tTested = %f, testedShift = %f\n',answer, tTest, testedShift)
