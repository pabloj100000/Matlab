function ProceduralGarboriumDemo()
% ProceduralGarboriumDemo([2=200]) -- An aquarium full of cute little procedural gabors!
%
...
%
% Each frame of the animation contains '2' patches, 2 defaults
% to 200. Change the number as first optional parameter '2' if you want
...
screen.backColor = 128;
[screen.w screen.rect] = PsychImaging('OpenWindow', screenid, screen.backColor);
...
% Initialize matrix with spec for all '2' patches to start off
% identically:
...
gabortex = CreateProceduralGabor(screen.w, tw, th, 1, -0*[1 1 1 0]);
...
while ~KbCheck
    ...
    Screen('DrawTextures', screen.w, gabortex, [], dstRects, rotAngles, [], [], [], [], kPsychDontDoRotation, mypars);
    ...
    rotAngles = rotAngles + 1 * randn(1, 2);
    ...
    vbl = Screen('Flip', screen.w, vbl + 0.5 * ifi);
...
% {    
    for i=1:boxesN
        ...
        array(left:right, top:bottom) = rand()-.5;%color;
    end
