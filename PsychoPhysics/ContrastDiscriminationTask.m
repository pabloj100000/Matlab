...
try
    ...
    p.addParamValue('pedestal', .15, @(x) all(x>=0 & x<=1)); % contrast of the reference patch
    ...
    RIGHT = 2;
    LEFT = 1;
    minPhysicalThresh = log(1/127);
    maxPhysicalThresh = log(1-pedestal);
    DiffSign = 1;
    % *************** End of Constants ******************
    ...
    % Get object texture
    [pedestalTex pedestal] = GetGaborText2(objSize, lambda, sigma, phase, pedestal);
    ...
    Screen('DrawText', screen.w, 'Contrast Task', 0, 0, 0, 127);
    Screen('DrawText', screen.w, 'Your job is to select the highest contrast patch in each trial', 0, 20, 0, 127);
    ...
    for i=1:trialsPerDelay
        ...
        for j=1:objDelayN
            ...
            if (DiffSign==-1)
                tTest = min(tTest, log(pedestal));
            end
            pedestalPatch = randi(2, 1, 1);      % pedestalPatch = 1=> left 
                                                 %                 2=> right

            % grab a gabor patch of the suggested contrast
            intendedContrast = exp(tTest)*DiffSign+pedestal;
            if (abs(intendedContrast) < .001)
                intendedContrast = 0;
            elseif (intendedContrast < 0)
                error 'intendedContrast was smaller than 0';
            end
            
            [testTex actualContrast] = GetGaborText2(objSize, lambda, sigma, phase, intendedContrast);
            tTested = log(actualContrast-DiffSign*pedestal);
            ...
            while 1
                ...
                if (keyIsDown)
                    ...
                    elseif ((keyCode(RIGHT_BTN) || keyCode(RIGHT_SHIFT)) && higestPatch==RIGHT) || ...
                        ...
                        % Update the pdf
                        answer = 'Right';
                        q(delayOrder(j))=QuestUpdate(q(delayOrder(j)),tTested, true); % Add the new datum (actual test intensity and observer response) to the database.
                        break
                    elseif ((keyCode(RIGHT_BTN) || keyCode(RIGHT_SHIFT)) && higestPatch==LEFT) || ...
                        ...
                        % Update the pdf
                        answer = 'Wrong';
                        q(delayOrder(j))=QuestUpdate(q(delayOrder(j)),tTested,false); % Add the new datum (actual test intensity and observer response) to the database.
                        break
                    end
                end
            end
            fprintf('trial %d, delay %d\n', i, j);
            fprintf('\t%s, tTested = %f, intendedContrast = %f, acutalContrast = %f\n',answer, tTested, intendedContrast, actualContrast)
