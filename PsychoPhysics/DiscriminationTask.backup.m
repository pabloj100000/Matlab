...
try
    ...
    g.size = p.Results.gSize;
    g.freq = p.Results.gFreq;
    g.sc = p.Results.gSc;
    g.phase = p.Results.gPhase;        % in radians, 2? = 0
    g.contrast = p.Results.gContrast;       % from 0 to 1
    ...
    % identically:
    mypars = repmat([g.phase+180, g.freq, g.sc, g.contrast, g.aspectratio, 0, 0, 0]', 1, 2);
    
    % Build a procedural gabor texture for a gabor with a support of tw x th
    % pixels, and a RGB color offset of 0.5 -- a 50% gray.
    gabortex = CreateProceduralGabor(screen.w, g.size, g.size, 1, -.0*[1 1 1 0]);
    ...
    for trials=1:trialsN
        for k=1:2
            ...
            for i=1:trialsPerDelay
                ...
                tTest = [0 QuestQuantile(q(condition))];	% Recommended by Pelli (1987), and still our favorite.
