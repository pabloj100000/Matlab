function S = UFlickerObj(varargin)
...
%SaveBinary(contrastSeq, 'uint8')
    ...
    S{checkersN,contrastsN} = {};
    for i=1:checkersN
        for j=1:contrastsN
            S{i,j} = RandStream('mcg16807', 'Seed',objSeed + i -1);
        end
    end
    ...
    for presentation = 1:presentationsN
        ...
        if (size(objContrasts,1)>1)
            presentationContrasts = contrastSeq(:, presentation);
        else
            presentationContrasts(1:checkersN) = contrastSeq(1, presentation);
        end
        ...
        for back=1:4
            if (back==1)
                ...
                for checker=1:checkersN
                    ...
                    normDistribution(checker,:) = randn(S{checker,presentationContrasts(checker)}, 1, objJumpsPerPeriod);
                end
                
                % convert the normDistribution to intensity values taken
                % the contrast and mean into account.
                for i=1:checkersN
                    contrast = objContrasts(presentationContrasts(i));
                    if (contrast==1)
                        objSeq(i,:) = uint8(normDistribution(i,:)>0)*255;
                    else
                        objSeq(i,:) = uint8(normDistribution(i,:)*contrast(i)*objMeans(i) + objMeans(i));
                    end
                end
