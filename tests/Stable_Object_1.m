% File executed on: 05-Jul-2010 09:02:49
function Stable_Object()
...
objSize = 8*barsWidth;          % in pixels

backContrast = 40/100;       %mean is 127
reversePeriod = 1;        % in seconds
objIntensities = 1:4;
objIntensities = objIntensities.*64;
objIntensities = objIntensities-mean(objIntensities)+127;

consecPresentations = 11;
consecPresentationLength = reversePeriod*consecPresentations;

% Probably you do not want to mess with these
waitframes = 1;
movieDurationSecs=1500;  % in seconds
RFlength = 0;          % in seconds
seed = 1;

% Redefine exp time to have an even number of jitters
movieDurationSecs = consecPresentationLength* ...
    floor(movieDurationSecs/(consecPresentationLength));
...
try
    ...
    jumpsPerPeriod = round(reversePeriod*framesPerSec);
    ...
    % keypress.
    consecPresentationsN = movieDurationSecs/consecPresentationLength;

    % make the random sequence of object intensities to display

    rand('seed', seed);
    objSeq = randperm(consecPresentationsN);
    ...
    framesN = consecPresentationLength*60;
    
    % Animationloop:
    % start with some RF
    rand('seed', 1)
    [exitFlag vbl] = BinaryCheckers(screen, RFlength*framesPerSec, waitframes, 35, 35, .2, backRect, pd, vbl);
    for consecPresentation = 1:consecPresentationsN

        stim = objSeq(consecPresentation);
        ...
        JitteringBackTex_UniformFieldObj(backSeq, objColor, screen, ...
            waitframes, framesN, backTex, backRect, backSource, objRect, vbl, pd)
    ...
    if (consecPresentation == consecPresentationsN)   % if it got here without any key being pressed
        empty=[];
...
function [structIndex, fieldNames, values] = StructList2cellList(varargin)
    ...
    time = toc
endfunction vbl = Wait2Start(mywindow, debugging, varargin)
    if (nargin>2)
        text=varargin{1};
    else
        text='';
    ...
    screenNumber = max(Screen('Screens'));
    if (debugging || screenNumber>0)
        vbl = Wait4Button2(mywindow, text);
    else
        vbl = WaitForRecComp(mywindow, text);
    end
end

function vbl = Wait4Button(k, varargin)
    KbName('UnifyKeyNames');
    while 1
...
function vbl = WaitForRecComp(winPtr, varargin)
        ...
        Screen(winPtr, 'DrawText', 'Waiting for rec computer',200,200, 0 );
        if (nargin>0)
            Screen(winPtr, 'DrawText', [num2str(varargin{1})],250,300, 0 );
        end
