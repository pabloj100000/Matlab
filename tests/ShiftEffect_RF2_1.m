% File executed on: 06-May-2010 11:06:52
function ShiftEffect_RF2_1()
% Divide the screen in object and background.
% Object will be a uniform field box of random intensity.
% Backk will be a grating of a giving contrast and spatial frequency.
% Back gratings will be reversing at a given frequency
% The background contrast will change from 0 to 40% every switchTime

% Change this parameters as needed
debugging = 0;

stimSize = 600;         % in pixels
objSize = 160;          % in pixels
barsWidth = 16;        % in pixels

backContrast = 40/100;       %mean is 127
objContrast = 16/100;

backConditions = 3;

reverseFreq = 1;      % in Hz
switchTime = 100;       %in seconds
jumpSize = barsWidth/2;   

% Probably you do not want to mess with these
seed = 1;
waitframes = 1;
movieDurationSecs=1500;  % in seconds

% Redefine exp time to have an even number of switchTime periods
movieDurationSecs = backConditions*switchTime*...
    floor(movieDurationSecs/(backConditions*switchTime));
...
try
    ...
    % how often do I switch between the 2 backgrounds?
    switchingFrame = switchTime*60;
    ...
    % make the sequence of steps for the background shift
    shiftSteps = zeros(1, 2*reverseFrame);
    shiftSteps(1:6)=1;
    shiftSteps(31:36)=-1;

    % Perform initial Flip to sync us to the VBL and for getting an initial
    % VBL-Timestamp for our "WaitBlanking" emulation:
    vbl=Wait4Button2(screen.w, ['Recotrd for ', num2str(movieDurationSecs), ' secs']);

    % init variables that will change in the loop
    backPhase = 0;
    shiftRect = [0 0 0 0];

    objLight = screen.gray*(1-objContrast);
    objDelta = 2*screen.gray*objContrast;
    ...
    while (frame < framesN) & ~KbCheck %#ok<AND2>
        ...
        if (mod(frame, switchingFrame)==0)           
            % changing from 1 back to the other, but into which one?
            if (mod(frame, backConditions*switchingFrame)==0)    
                % showing back1
                nextSeed = rand('seed');
            else
                % showing back2
        ...
        background = mod(floor(frame/switchingFrame),backConditions);
        switch background
            case 0
                % Background changes phase every 'switchFrame' frames
                if (mod(frame, reverseFrame) == 0)
                    backPhase = jumpSize - backPhase;
                    shiftRect = backPhase*[1 0 1 0];
                end
            case 1
                % shifting background
                shiftRect = shiftRect + shiftSteps(mod(frame, 2*reverseFrame)+1)*[1 0 1 0];
            case 2
                % do nothing
        end
        ...
        objColor = objLight + (rand()>.5)*objDelta;
