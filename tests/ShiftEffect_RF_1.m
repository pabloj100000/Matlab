...
backContrast = [0 .4 4 16 40]/100;       %mean is 127
objContrast = 16/100;
...
% Redefine exp time to have multiple of 100 sec per contrast
movieDurationSecs = size(backContrast,2)*100*...
    floor(movieDurationSecs/(100*size(backContrast,2)));
...
%    bars = ceil(mod(floor(x/barsWidth),2)*2*gray*objContrast+gray*(1-objContrast));
%    objTex = Screen('MakeTexture', w, bars);
%    backTex = 1;            % dummy value to be deleted inside the loop
    ...
    % how many frames do I stay in each contrast before jumping to the next one.
    framesPerContrast = floor(framesN/size(backContrast,2));  
    ...
    while (frame < framesN) & ~KbCheck %#ok<AND2>
        ...
        % contrasts
        if (mod(frame, framesPerContrast)==0)
