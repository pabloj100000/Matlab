% File executed on: 27-May-2010 11:26:45
function fillingRF()
% Obj Parameters
...
%                                2 Pads          1 Pad
padSizeV = 10*objChecker;                  %60;             140
padSizeH = 10*objChecker;                  %43;             140
objPadN = 1;                     %2;              1
objPadsDist = 0;                 %92;             0
deltaY = 0;
deltaX = 0;
...
%    backColor2 = mod(x+1,2)*gray*backContrast+gray*(1-backContrast)
    backColor1 = mod(x,2)*255;
    backColor2 = 255-backColor1;
...
function SaveBinary(obj)
    nameout = [inputname(1), '.bin'];
    fid = fopen(nameout, 'w');
    type = whos(inputname(1));
    fwrite(fid, obj, type.class);
    fclose(fid);
end
...
function vbl = WaitForRecComp(mywindow, varargin)
        Screen('FillRect', mywindow, 127);
        Screen(mywindow,'TextSize', 36);
        Screen(mywindow, 'DrawText', 'Waiting for rec computer',200,200, 0 );
        if (nargin>0)
            Screen(mywindow, 'DrawText', ['(Record for a bit more than ',...
                num2str(varargin{1}), ' secs)'],250,300, 0 );
        end
