...
try
    ...
    if (isempty(backTex))
        ...
        if (backPattern)
            ...
            backTex = GetBarsTex(checkersN+1, 1, backContrast);
        end
    end
    ...
    % make a random sequence of objects to display. I want to display all
    % 'objectsN' objects before repeating one. Therefore I randomize groupd
    % of 'objectsN' at a time.
...
function p =  ParseInput(varargin)
    ...
    stimSize = floor(SCREEN_SIZE/PIXELS_PER_100_MICRONS)*PIXELS_PER_100_MICRONS;
    stimSize(1)=[];
    
    % General
    p.addParamValue('stimSize', stimSize, @(x)x>0);
    p.addParamValue('presentationLength', 45, @(x)x>0);         % 3Hours / (45s *16 objects) = 15 presentations per object
    ...
    p.addParamValue('objRect', GetRects(12*PIXELS_PER_100_MICRONS, screenCenter), @(x) size(x,2)==4);
