function newSeed = OMS_Saccading(varargin)
...
%    backDestOri = OffsetRect(backDestOri,screen.rect(1)+2*checkersSize,screen.rect(1)+2*checkersSize)  
    ...
    % reverse periphery every 0.5s
    peripheryFrame = floor(screen.rate*0.5/waitframes);
peripheryFrame
waitframes
screen.rate
    % Define the PD box
    ...
    for frame=0:framesN-1
        ...
        if (mod(frame, peripheryFrame)==0)
            offsetPeriphery = mod(offsetPeriphery + peripheryStep, 2*peripheryStep);
        end
