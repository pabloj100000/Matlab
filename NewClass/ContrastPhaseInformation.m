...
%}
        % { 
        for i = 1:contrastsN
            startIndex=floor(framesN/objFramePeriod)*(presentation-1);
            for frame=0:framesN-1
                ...
                if (mod(frame, objFramePeriod)==0)
                    ...
                    output(contrastSeq(i), startIndex) = phaseIndex;
...
% {
            if exist('A', 'var')
                B=hist(A,16);
                if min(B) ~= max(B)
                    fprintf('problem with presentation %d. Contrast = %d\n', presentation, contrastSeq(i));
                else
                    fprintf('presentation %d. Contrast = %d\n', presentation, contrastSeq(i));
                end
            end
%}
            if (KbCheck)
