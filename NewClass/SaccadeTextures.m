function SaccadeTextures(varargin)
% Define a texture and show it at a given alpha value on top of a given
% luminance level.
global screen
try    
    ...
    % round stimSize to be an integer number of PIXELS_PER_100MICRONS
    stimSize = round(stimSize/PIXELS_PER_100_MICRONS)*PIXELS_PER_100_MICRONS;
    
    LoadAllTextures(images2load, [stimSize, stimSize]);
    
    % Get axess to local folder with natural scenes
    ...
    % generate 1 array for the periphery. 
    if (periIndex >=0)
        peripheryIm = imread([folder, 'image', num2str(periIndex),'.jpg']);
        peripheryIm = peripheryIm(1:stimSize, 1:stimSize);
    else
        checkersN = stimSize/checkersSize;
        peripheryIm = GetCheckers(checkersN, checkersN, checkersSize);
    end    

    % generate 1 array for the object
    if (objIndex >=0)
