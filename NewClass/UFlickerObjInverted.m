function backSeed = UFlickerObjInverted(varargin)
...
try
    ...
    objRects = zeros(length(objSize),4);
    for i=1:length(objSize)
        objRects(i,:) = GetRects(objSize(i), objCenter(i,:));
    end
    ...
    for presentation = 1:presentationsN
        ...
        for frame = 0:waitframes:framesN-1
            ...
            for i=1:length(objSize)
                Screen('FillRect', screen.w, objColor, objRects(i,:))
            end
            ...
            % Write some info
            Screen(screen.w, 'DrawText', ['frame = ', num2str(frame), '/', num2str(framesN)],1,1, screen.black );
            Screen(screen.w, 'DrawText', ['presentation = ', num2str(presentation), '/', num2str(presentationsN)],1,10, screen.black );
...
function p =  ParseInput(varargin)
    ...
    % General
    p.addParamValue('presentationLength', 5, @(x)x>0);
    p.addParamValue('movieDurationSecs', 100, @(x)x>0);
    p.addParamValue('waitframes', 1, @(x)isnumeric(x)); 
    p.addParamValue('pdStim', 104, @(x) isnumeric(x));
    
    % Background related
    p.addParamValue('backContrast', .03, @(x)x>=0 && x<=1);
    p.addParamValue('backSeed', 1, @(x) isnumeric(x));
    p.addParamValue('backSize', screenY, @(x) x>0);
 
    % Object related
    p.addParamValue('objContrast', 1, @(x) all(all(x>=0)) && all(all(x<=1)));
    p.addParamValue('objSize', [4 8], @(x) all(all(x))>0);
    p.addParamValue('objCenter', [screenCenter; screenCenter+200], @(x) size(x,2)==2);
